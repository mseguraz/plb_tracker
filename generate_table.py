            ...
            for col_name in col_names:
                if col_name in columns:
                    if row_name in columns[col_name]:
                        str_value, int_value, condition, mask = columns[col_name][row_name]
...
def generate_excel_tables_audit(data: dict, output_file: str, bin_dict: dict):
    ...
    #get column names
    col_names = ['INSTANCE', 'BIN', 'PLIST', 'PARTITIONS', 'GROUPING', 'PHASES', 'REVISION', 'CHUNKS']
    ...
    for names in data:
        ...
        for instance, columns in data[names].items():
            ...
            for i in range(len_instance):
                row=["", col1[i], col_12[i], col2[i], col3[i], col4[i], col5[i], col6[i], col7[i]]
                ws.append(row)
