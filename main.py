...
from generate_table import generate_excel_tables, generate_excel_tables_audit
from get_plist import get_plist, contains_exact_word, get_plist_audit
from chop_bypass import process_map, is_executed, get_plist_with_rules
...
# Get paths of all files to be used for the table for class
def filepaths_iodie_class_linux(tp_path):
    global plist_path, class_mtpl_path
    tp_env_path=tp_path+'/POR_TP/CLASS_TP'
    ...
    with open(env_path, 'r') as file:
        for line in file:
            if line.startswith(f'MSCNIO{process[0:3].upper()}_PATMODIFY_PATH'):
            ...
            if f'MSCNIO{process[0:3].upper()}_PATMODIFY_PATH' in line and '.patmod' in line:
                ptd=line.split('\\', 1)
                ptd=ptd[1].strip().strip('" ;+')
                patmods.append(ptd)
    ...
    with open(plist_all_path, 'r') as file:
        for line in file:
            if 'scan' in line and 'io_class' in line:
                parts = line.split('=', 1)
    ...
    # mtpl paths
    class_mtpl_path = tp_path + f'/Modules/SCN_UNCORE_IO/{IO_CLASS_MTPL}'

# Get paths of all files to be used for the table for class
def filepaths_iodie_class_windows(tp_path):
    global plist_path, class_mtpl_path
    tp_env_path=tp_path+'\\POR_TP\\CLASS_TP'
...
def filepaths_iodie(tp_path):
    ...
    with open(env_path, 'r') as file:
        for line in file:
            if line.startswith('MSCNIO_PATMODIFY_PATH'):
            ...
            #for patmod
            if 'SCN_UNCORE_IO' in line and '.patmod' in line:
                ptd=line.split('\\', 1)
                ptd=ptd[1].strip().strip('" ;+')
                patmods.append(ptd)
    ...
    with open(plist_all_path, 'r') as file:
        for line in file:
            if 'scan' in line and 'io_sort' in line:
                parts = line.split('=', 1)
    ...
    if not PATMOD:
        for p in patmods:
            patmods_path=os.path.join(tp_path, p)
            print(patmods_path)
            get_patmod(patmods_path)

    # mtpl paths
    uncore_mtpl_path = tp_path + f'\\Modules\\SCN_UNCORE_IO\\{IO_MTPL}'
    hvqk_mtpl_path = tp_path + f'\\Modules\\SCN_UNCOREHVQK_IO\\{IO_HVQK_MTPL}'
...
def get_dut_flow(flow, mtpl_file):
    ...
    for line in mtpl_file:
        if line.startswith('DUTFlow') and flow in line: #mseguraz: change for DMR tos4
            cond=True
        if cond:
...
def extract_data(input_str): 
    ...
    match = re.search(r'_([^_]+)_edt', input_str)
    ...
    if 'chain' in input_str:
        ...
        return result, rev, region, masking
    ...
    # ph1, ph2 or ph3
        ...
        if not SEPARATE:
            ...
            if region[p_position+3:]:  #If there is topoff
                phase=region[p_position:p_position+3]
                if phase=='ph1':
                    return result, rev, region[p_position+3:], masking
                else:
                    region=region.replace('ph1', '')
                    return result, rev, region[p_position:], masking
            else:
                return result, rev, region[p_position:p_position+3], masking
            ...
            if 'trans' in input_str:
                if region[p_position+3:]:  #If there is topoff
                    phase=region[p_position:p_position+3]
                    if phase=='ph1':
                        return result, rev, region[p_position+3:]+'_tatpg', masking
                    else:
                        region=region.replace('ph1', '')
                        return result, rev, region[p_position:]+'_tatpg', masking
                else:
                    return result, rev, region[p_position:p_position+3]+'_tatpg', masking
                
            else:
                if region[p_position+3:]:  #If there is topoff
                    phase=region[p_position:p_position+3]
                    if phase=='ph1':
                        return result, rev, region[p_position+3:]+'_atpg', masking
                    else:
                        region=region.replace('ph1', '')
                        return result, rev, region[p_position:]+'_atpg', masking
                else:
                    return result, rev, region[p_position:p_position+3]+'_atpg', masking
...
def get_dutflow_items(flow, mtpl_file):
    ...
    print ("Flow Instances", flow_instances)
            ...
            for line in mtpl_file:
                ...
                if cond and 'BypassPort' in line:
                    ...
                    if SORT and '-1' not in line:
                        ...
                        if not is_executed(line, process, FUSED):
                            print("This instance is Bypass, FUSED switch used:", instance)
                            print(line)
                            bypassed=True
                            break
                ...
                if cond and ('Patlist' in line or 'patlist' in line) and '"Patlist"' not in line:
                    ...
                    if occurrences >= 2:
                        print("Patlist with several rules")
                        value=get_plist_with_rules(line, process, FUSED)
                        to_append=[value, EDC]

                    elif occurrences > 0 and IODIE and SORT:
                        value=get_plist_with_rules(line, 'ucc_hot', FUSED, DSTEP)
                        to_append=[value, EDC]
                ...
                if cond and line.startswith('}'):
                    ...
                    bypassed=False
                    break
        #print("Plist:", plist, "for isntance:", f)    
    plbs={}
    for p in plist:
        ...
        for i, pp in enumerate(plbs_list):
            ...
            flavor, rev, region, mask =extract_data(pp) 
            #print(flavor, rev, region)
...
def check_inputs(args):
    ...
    if args.f.lower() == 'sort':
        ...
        if args.iodie:
            global DSTEP
            step= input("For GNR SORT, IODIE can run in either DSTEP or ESTEP. Please select the one you want:\n")
            if step.lower()=='estep':
                DSTEP=1
...
def main(args): 
    ...
    parser.add_argument("-iodie", action="store_true", help="For IO die")
    parser.add_argument("-fused", action="store_true", help=" For IODIE. Fused unit (unit not fused by default)")
    ...
    #for scan by default
    if SORT:
        filepaths(args.tp) if not IODIE else filepaths_iodie(args.tp)
    else:
        if 'dows' in os.getenv('OS'):
            filepaths_class_windows(args.tp) if not IODIE else filepaths_iodie_class_windows(args.tp)
        else:
            filepaths_class_linux(args.tp) if not IODIE else filepaths_iodie_class_linux(args.tp)
    ...
    if SORT:
        if IODIE:
            for region in FLOWS_IODIE:
                for flow in FLOWS_IODIE[region]:
            ...
            for region in FLOWS_IODIE_HVQK:
                for flow in FLOWS_IODIE_HVQK[region]:
        ...
        if IODIE:
            for region in FLOWS_IODIE_CLASS:
                for flow in FLOWS_IODIE_CLASS[region]:
                    #print(region, flow)
    ...
    if IODIE:
        generate_excel_tables(plbs, args.table, NAMES_IO) if SORT else generate_excel_tables(plbs, args.table, NAMES_IO_CLASS)
    else:
